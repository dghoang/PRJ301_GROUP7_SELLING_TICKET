        ...
        <div class="col-lg-10">
            ...
            <div class="glass-gradient rounded-4 p-4 mb-4 position-relative overflow-hidden animate-fadeInDown">
                <div class="row align-items-center">
                    ...
                    <div class="col-md-6 text-end d-none d-md-flex justify-content-end align-items-center gap-2">
                        <form id="eventFilterForm" action="${pageContext.request.contextPath}/organizer" method="get" class="m-0">
                            <select name="eventId" class="form-select bg-glass border-glass rounded-pill px-3 text-white shadow-none" onchange="this.form.submit()" style="min-width: 200px; max-width: 300px; background-color: rgba(255,255,255,0.1); backdrop-filter: blur(10px);">
                                <option value="0" class="text-dark">💥 Tất cả sự kiện</option>
                                <c:forEach var="ev" items="${myEvents}">
                                    <option value="${ev.eventId}" class="text-dark" ${selectedEventId == ev.eventId ? 'selected' : ''}>
                                        ${ev.title}
                                    </option>
                                </c:forEach>
                            </select>
                        </form>
                        <a href="${pageContext.request.contextPath}/organizer/create-event" class="btn btn-gradient rounded-pill px-4 hover-glow text-nowrap">
                ...
                <div class="col-lg-8 animate-on-scroll">
                    <div class="card glass-strong border-0 rounded-4 h-100">
                        <div class="card-header bg-transparent border-0 d-flex justify-content-between align-items-center pt-4 px-4">
                            ...
                            <div class="btn-group btn-group-sm" id="salesPeriodGroup">
                                <button class="btn btn-outline-primary rounded-start-pill active" onclick="loadSalesChart(7, this)">7 Ngày</button>
                                <button class="btn btn-outline-primary" onclick="loadSalesChart(30, this)">30 Ngày</button>
                                <button class="btn btn-outline-primary rounded-end-pill" onclick="loadSalesChart(90, this)">90 Ngày</button>
                            </div>
                        </div>
...
function loadSalesChart(days, btnElement) {
    if (btnElement) {
        document.querySelectorAll('#salesPeriodGroup .btn').forEach(b => b.classList.remove('active'));
        btnElement.classList.add('active');
    }

    const eventIdParam = new URLSearchParams(window.location.search).get('eventId');
    let url = document.body.dataset.contextPath + '/organizer/dashboard/chart-data?type=revenue&days=' + days;
            ...
            salesChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: { position: 'left', grid: { color: 'rgba(0,0,0,0.04)' }, ticks: { font: { family: 'Inter' }, precision: 0 }, beginAtZero: true },
                        y1: { position: 'right', grid: { display: false }, ticks: { font: { family: 'Inter' }, callback: function(value) { return value.toLocaleString('vi-VN'); } }, beginAtZero: true }
                    }
                }
            });
        })
...
function loadTicketTypeChart() {
    const eventIdParam = new URLSearchParams(window.location.search).get('eventId');
    let url = document.body.dataset.contextPath + '/organizer/dashboard/chart-data?type=tickets';
    if (eventIdParam) url += '&eventId=' + eventIdParam;
            ...
            ticketTypeChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: counts.length > 0 ? ['#9333ea', '#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#ec4899'] : ['#e2e8f0'],
                        borderWidth: 0,
                        hoverOffset: counts.length > 0 ? 8 : 0
                    }]
                },
                options: {
                    responsive: true,
                    ...
                    cutout: '65%',
                    plugins: {
                        legend: { display: counts.length > 0, position: 'bottom', labels: { usePointStyle: true, padding: 15, font: { family: 'Inter', size: 12, weight: '500' } } },
                        tooltip: { enabled: counts.length > 0, backgroundColor: 'rgba(255,255,255,0.95)', titleColor: '#1e293b', bodyColor: '#64748b', borderColor: 'rgba(0,0,0,0.05)', borderWidth: 1, cornerRadius: 12, padding: 12, boxPadding: 6 }
                    }
                }
            });
        })
        .catch(err => console.error('Failed to load ticket type chart', err));
}
</script>
