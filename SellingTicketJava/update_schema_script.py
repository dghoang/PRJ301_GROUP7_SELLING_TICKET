...
events_end_target = "PRINT 'Table Events created.';\nEND\nGO"
...
if events_end_target in content_norm:
    content_norm = content_norm.replace(events_end_target, events_replacement)
else:
    print("Could not find Events end target")

# 2. Inject Orders ALTER statement
orders_end_target = "PRINT 'Table Orders created.';\nEND\nGO"
...
if orders_end_target in content_norm:
    content_norm = content_norm.replace(orders_end_target, orders_replacement)
else:
    print("Could not find Orders end target")

# Write back preserving Windows line endings
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content_norm.replace('\n', '\n')) # Python text mode translations handle \n to \r\n automatically on Windows
